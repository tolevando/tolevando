...
class PaymentAPIController extends Controller
{
    ...
    public function byMonth()
    {
        $payments = [];
        if (!empty($this->paymentRepository)) {
            $payments = $this->paymentRepository->join("orders", "payments.id", "=", "orders.payment_id")->where('orders.active','1')->where('payments.status','!=','Não Pago')->orderBy("payments.created_at",'asc')->select('payments.*')->get()->map(function ($row) {
                $row['month'] = $row['created_at']->format('M');
                return $row;
            })->groupBy('month')->map(function ($row) {
                return number_format($row->sum('price'),2,'.','');
            });
        }
