...
class UserAPIController extends Controller
{
        ...
        try {
            ...
            if (auth()->attempt(['email' => $request->input('email'), 'password' => $request->input('password')])) {
                ...
                if (!$user->hasRole('entregador')) {
                    $this->sendError('Usuário não é entregador', 401);
                }
    ...
    function sendResetLinkEmail(Request $request)
    {
        ...
        if ($response == Password::RESET_LINK_SENT) {
            return $this->sendResponse(true, 'Link de Redefinição foi Enviado com Sucesso');
        } else {
            return $this->sendError([
                'error' => 'Link de Redefinição não foi enviado',
                'code' => 401,
            ], 'Link de redefinição não foi enviado');
        }
    ...
    public function getDisponibilidade(Request $request){
        $user = $this->userRepository->findByField('api_token', $request->input('api_token'))->first();        
        $driver = $this->driverRepository->findByField('user_id', $user->id)->first();
        ...
        */
        $settings = array();
        $settings['disponivel'] = ($driver->available??'0')==1;

        return $this->sendResponse($settings, 'Aberto/Delivery retrieved successfully');

    }

    public function setDisponibilidade(Request $request){
        $user = $this->userRepository->findByField('api_token', $request->input('api_token'))->first(); 
        //dd($user);       
        $driver = $this->driverRepository->findByField('user_id', $user->id)->first();
        
        $driver->available = $request->input('disponivel');
    ...
    public function checaNovoPedido(Request $request){
        ...
        WHERE o.driver_id = ".$user->id." and o.active = 1 and o.order_status_id in (3,4) and o.delivery_address_id is not null
