...
@if(isset($option) && isset($option->product_id))
  <!-- Product Id Field -->
  <div class="form-group row ">
    ...
    <div class="col-9">
      ...
      <div class="form-text text-muted">Editar produto</div>
    </div>
  </div>
@else
  <!-- Product Id Field -->
  <div class="form-group row">
    {!! Form::label('product_ids[]', trans("lang.option_product_id")."(s)",['class' => 'col-3 control-label text-right']) !!}
    <div class="col-9">
      {!! Form::select('product_ids[]', $product, null, ['class' => 'select2 form-control', 'multiple'=>'multiple']) !!}
      <div class="form-text text-muted">A opção será replicada para cada produto selecionado</div>
    </div>
  </div>
@endif
