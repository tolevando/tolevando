...
Route::get('testNotification/{id}','API\OrderAPIController@testNotification');
...
Route::get('markets/brands/{id}', 'API\MarketBrandsAPIController@returnBrands');
...
Route::middleware('auth:api')->group(function () {
    Route::group(['middleware' => ['role:entregador']], function () {
        Route::prefix('driver')->group(function () {
            ...
            Route::resource('faq_categories', 'API\FaqCategoryAPIController');
            Route::resource('faqs', 'API\FaqAPIController');
            Route::get('users/disponibilidade', 'API\Driver\UserAPIController@getDisponibilidade');
            Route::get('users/altera_disponibilidade', 'API\Driver\UserAPIController@setDisponibilidade');
            Route::get('users/checa_novo_pedido', 'API\Manager\UserAPIController@checaNovoPedido');
        });
    });
    Route::group(['middleware' => ['role:gerente']], function () {
        Route::prefix('manager')->group(function () {
            Route::get('users/aberto_and_delivery', 'API\Manager\UserAPIController@getAbertoAndDelivery');
            Route::get('users/altera_aberto_and_delivery', 'API\Manager\UserAPIController@setAbertoAndDelivery');
            Route::get('users/checa_novo_pedido', 'API\Manager\UserAPIController@checaNovoPedido');
            Route::post('users/{id}', 'API\UserAPIController@update');
